...
files = glob.glob("raw/res.country.state.municipio.csv")
...
def ConvertToXML():
    for i in range(len(files)):
        ...
        csvFile = files[i]
        xmlFile = 'res_country_state_municipio.xml'
        ...
        for row in csvData:
                print(row)
                xmlData.write("\t\t"+'<record id ="' +row[0]+'" model="res.country.state.municipio" forcecreate="0" >\n')
                xmlData.write("\t\t\t"+'<field name="name">' + row[1]+"</field>\n")
                xmlData.write("\t\t\t"+'<field name="state_id" ref="'+row[2]+'"/>'+"\n")
                xmlData.write("\t\t\t"+'<field name="ciudad_id" ref="'+row[3]+'"/>'+"\n")
